...
string SumOddNumbersInArr(params int[] arr)
{
    ...
    return sum == 0 ? "arrayda tek eded yoxdur" : $"Sum is {sum}";
}
#endregion
#region task-3
...
string EvenNumbersInArr(params int[] arr)
{
    ...
    if (count == 0)
    {
        return $"arrayda cut eded yoxdur";
    }
    else
    {
        foreach (int index in arr)
...
string ResultOfArr(params int[] arr)
{
    ...
    return total == 1 ? $"arrayda istenilen eded yoxdur" : $"Total is {total}";
}
#endregion
#region task-7
...
string PowOfEven(params int[] arr)
{
    ...
    return sum == 0 ? $"arrayda cut eded yoxdur " : $"Power of sum is {sum * sum}";
}
#endregion
#region task-8
